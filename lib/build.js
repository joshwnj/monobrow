...
module.exports = function (w, opts, vendorStreams) {
  ...
  */
  function writeVendorFileThenBundle () {
    const vendorDestPath = path.join(opts.rootDir, opts.output.dir, opts.output.vendor)
    multistream(vendorStreams)
  ...
  function bundle () {
    ...
    if (!shouldSplitVendor && vendorStreams.length) {
      multistream(vendorStreams.concat(bStream))
        .pipe(outStream)
    } else {
